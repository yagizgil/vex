...
impl CompilerApp {
    ...
    pub fn run(&mut self) {
        ...
        if self.args.stats {
            self.metrics.push(self.collector.end_phase("Loading"));
        }
        ...
        if self.args.inspect {
            #[cfg(not(feature = "inspector"))]
            {
                ...
                println!(
                    "{}",
                    "Warning: Inspector requested but not compiled in.".yellow()
                );
        ...
        if self.args.stats {
            self.metrics.push(self.collector.end_phase("Lexing"));
        }
        ...
        let mut parser = vex_parser::parser::Parser::new(tokens);
        let _astt = parser.parse();
        if self.args.stats {
            self.metrics.push(self.collector.end_phase("Parsing"));
        }
